...
function redirectToHomeIfAdmin(req, res, next) {
    const usuario = req.session.usuario;
    if (usuario && usuario.role === 'admin') {
        return res.status(200).render('home', { error: 'Solo pueden acceder los usuario con rol USER o PREMIUM.', errorColor: 'red' });
    }
    next();
}
...
module.exports = { isAdmin, redirectToHomeIfAdmin,  isAdminOrPremium, };